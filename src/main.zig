...
const legacy_events = [_]struct { []const u8, []const u8} {
...
const events = [_]struct { []const u8, []const u8} {
...
pub fn main() !void {
    ...
    switch (args.len) {
        2 => { 
            ...
            const events_map = std.StaticStringMap([]const u8).initComptime(events);
            if (events_map.get(args[1])) |desc| {
                print("Event ID: {s}: {s}", .{args[1], desc});
            } else {
                print("Could not find event with ID {s}", .{args[1]});
            }
        },
        3 => {
            const events_map = std.StaticStringMap([]const u8).initComptime(legacy_events);
            if (std.mem.eql(u8, args[1], "--legacy")) {
                if (events_map.get(args[2])) |desc| {
                    print("Legacy event ID: {s}: {s}", .{args[2], desc});
                } else { print("Could not find legacy event with ID {s}", .{ args[2] }); }
            } else if (std.mem.eql(u8, args[2], "--legacy")) {
                if (events_map.get(args[1])) |desc| {
                    print("Legacy event ID: {s}: {s}", .{args[1], desc});
                } else { print("Could not find legacy event with ID {s}", .{ args[1] }); }
            }
        },
        else => { usage(); },
    }
